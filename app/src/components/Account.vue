...
import { ref } from 'vue';
import { updateUserName as directusUpdateUserName } from '~/composables/auth';
...
const updateUserName = async () => {
  ...
  const response = await directusUpdateUserName(credentials);
  if (response === 'succes') {
    msg.value = "Updated succesfully";
  } else if (response !==
    null) {
    msg.value = response;
  }
...
<template>
  <div class="bg-gray-800 p-6 mt-6 rounded-lg shadow-md w-full">
    ...
    <h2 class="text-2xl text-white clemt-4">{{ user.first_name }}</h2>
    ...
    <div class="p-5 mt-5 border rounded-3xl">
      ...
      <div>
        ...
        <button  @click.prevent="updateUserName"  class="text-slate-400 underline float-right">save</button>
      </div>
      <div>
        <input v-model="userPass" placeholder='New Password' type="password"
          class="text-black w-full px-auto -py-4 border-4 rounded-md focus:outline-none" />
          <p class="text-red-500 text-xs italic">{{ msg }}</p>
        <button class="text-slate-400 underline float-right">
          save
        </button>
      </div>
    </div>
